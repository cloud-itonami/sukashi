(ns sukashi.methods.social
  "social.cljc — 透かし (すかし) DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from sukashi's own
...
(defn draft-observation-post
  "A dry-run observation post over sukashi's own domain (Ad-tech supply-chain + delivery-infrastructure + fraud-network observatory. Datafies the programmatic advertising ecosystem from the PUBLIC…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "sukashi R0: live social posting is Council Lv6+ + operator + "
